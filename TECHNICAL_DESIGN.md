...
class ComplianceState(str, Enum):
    NON_COMPLIANT = "NON_COMPLIANT"         # Element matches Applicability but lacks PSets/Attrs
    PARTIALLY_COMPLIANT = "PARTIALLY_COMPLIANT" # PSets exist, but some values are missing/invalid
...
| **Method**                     | **Input**                   | **Returns**                    | **Implementation Detail**                                                 |
| ------------------------------ | --------------------------- | ------------------------------ | ------------------------------------------------------------------------- |
| **`check_compliance()`**       | `ids_store`                 | `List[ComplianceStatus]`       | **Core Logic:** Compares every element against every applicable IDS Spec. |
| **`get_compliance_summary()`** | `ids_store`                 | `List[ModelComplianceSummary]` | Aggregates data for the high-level project overview.                      |
...
The **Query Engine** (`ComplexQuery`) must now support a new category: `Compliance`.

- **Filter Category:** `Compliance`
    
- **Property Name:** `status` or `spec_name`
    
- **Value:** `NON_COMPLIANT`, `PARTIALLY_COMPLIANT`, etc.
    
- **Use Case:** User queries `(Category == "IfcWall") AND (ComplianceStatus == "PARTIALLY_COMPLIANT")` to find elements needing granular data.
    

---

## 9. Architectural Guardrails (Compliance Edition)

1. **On-the-Fly Analysis:** Compliance status should be calculated dynamically by comparing the IFC model to the IDS store. Do not rely on hardcoded "Status" properties in the IFC file, as they can become out of sync.
    
2. **The "Partial" Logic:** An element is `PARTIALLY_COMPLIANT` if the required Property Set and Property exist, but the current value is not among the `options` (enumerations) allowed by the IDS.
    
3. **Performance:** `check_compliance()` is a heavy operation. The engine should use an internal cache (mapping GUID -> Status) that is invalidated only when the model or IDS is modified.
