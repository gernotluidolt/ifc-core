...
def real_ids_path():
    return Path("tests/data/AT_3-1_Informationsanforderungskatalog für Hochbauprojekte_edited.ids")
...
def mock_ifc_store(tmp_path):
    ...
    project = ifcopenshell.api.run("root.create_entity", model, ifc_class="IfcProject", name="Sample Project")
    ...
    if not contexts:
         context_rc = ifcopenshell.api.run("context.add_context", model, context_type="Model")
         context = ifcopenshell.api.run("context.add_context", model, context_type="Model", 
                                        context_identifier="Body", target_view="MODEL_VIEW", parent=context_rc)
    else:
         context = contexts[0]
         
    representation_wall = ifcopenshell.api.run("geometry.add_wall_representation", model, 
        context=context, length=5.0, thickness=0.2, height=3.0)
    ifcopenshell.api.run("geometry.assign_representation", model, product=wall, representation=representation_wall)
    ifcopenshell.api.run("geometry.edit_object_placement", model, product=wall)
    
    # 4. Create a Slab
    slab = ifcopenshell.api.run("root.create_entity", model, ifc_class="IfcSlab", name="Sample Slab")
    ...
    ifcopenshell.api.run("geometry.assign_representation", model, product=slab, representation=representation_slab)
